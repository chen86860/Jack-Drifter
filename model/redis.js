    ...
    pool.acquire(function (err, client) {
        ...
        client.select(type[bottle.type], function () {
            ...
            client.HMSET(bottleId, bottle, function (err, result) {
                ...
                client.PEXPIRE(bottleId, 864000000 + bottle.time - Date.now())
...
//检查用户是否超过捡瓶子的次数限制
function checkThowTimes(owner, callback) {
    pool.acquire(function (err, client) {
        if (err) return callback({code: 0, msg: err});
        client.select(3, function () {
            ...
            client.get(owner, function (err, result) {
                if (err) return callback({code: 0, msg: err});
                if (result >= 10) {
                    return callback({code: 0, msg: "今天捡瓶子的机会已经用完啦～明天再来吧～"})
                }
                //增加捡瓶子的次数
                client.incr(owner, function () {
                    //检查是否是第一次扔瓶子
...
exports.throw = function (bottle, callback) {
    checkThowTimes(bottle.owner, function (result) {
        if (result.code === 0) {
